...
/* 
@PostMapping("/user/events/save/{eventId}")
public String saveEvent(@PathVariable Long eventId, Principal principal, RedirectAttributes redirectAttributes) {
    try {
        eventService.saveEventForUser(eventId, principal.getName());
        redirectAttributes.addFlashAttribute("successMessage", "Event saved successfully.");
    } catch (Exception e) {
        redirectAttributes.addFlashAttribute("errorMessage", "Error saving event: " + e.getMessage());
    }
    return "redirect:/user/home/userForum"; // Cambiado para redirigir a un endpoint válido
}
*/
